// app-canvas.jsx — wraps desktop App + mobile companion in a Design Canvas
// for side-by-side review

const { Icon: ACIcon } = window;
...
// ============ DESKTOP APP (artboard-constrained version of original App) ============
function DesktopApp({ tweaks, setTweak }) {
  ...
  return (
    <div style={{position:'relative',width:'100%',height:'100%',overflow:'hidden',
      background:'radial-gradient(ellipse 100% 70% at 30% 10%, rgba(34,211,238,0.10), transparent 60%), radial-gradient(ellipse 80% 60% at 80% 90%, rgba(167,139,250,0.10), transparent 60%), #080a10',
      borderRadius:14}}>

      <div className="app-shell" style={{position:'relative',height:'100%',inset:'auto'}}>
        {/* Titlebar */}
        ...
        {/* Body */}
        <div className="body-region" data-screen-label="Sceptre Estate · Desktop">
          {/* Sidebar */}
...
// ============ ROOT — DesignCanvas with desktop + mobile artboards ============
function CanvasRoot() {
  const [tweaks, setTweak] = window.useTweaks(TWEAK_DEFAULTS);

  // Apply accent hue tweak (recolors :root vars) — same as original App
  React.useEffect(() => {
    const map = {
  ...
  return (
    <>
      <window.DesignCanvas storageKey="sceptre-estate-canvas">
        <window.DCSection id="surfaces" title="Sceptre Estate" subtitle="Desktop workspace + mobile companion · same data, different surface">
          <window.DCArtboard id="desktop" label="Desktop · Web app" width={1480} height={900}>
            <DesktopApp tweaks={tweaks} setTweak={setTweak}/>
          </window.DCArtboard>
          <window.DCArtboard id="mobile" label="Mobile · iPhone 15 Pro" width={402} height={874}>
            <window.MobileApp/>
          </window.DCArtboard>
        </window.DCSection>
      </window.DesignCanvas>

      {/* Tweaks panel — applies to desktop accent + density */}
      <window.TweaksPanel title="Tweaks">
        ...
        <window.TweakSection title="Desktop layout">
