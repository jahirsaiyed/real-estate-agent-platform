#!/usr/bin/env python3
"""
Create Jira Subtasks for all stories in KAN project.
Stories KAN-19 to KAN-70 already exist.
Uses Subtask type (ID 10002) which is the only valid child of Story in this project.
"""
...
def create_subtask(summary: str, parent_key: str, labels: list) -> str:
    payload = {
        "fields": {
    ...
    if resp.status_code not in (200, 201):
        print(f"  ERROR '{summary[:60]}': {resp.status_code} {resp.text[:150]}")
        return ""
    key = resp.json()["key"]
...
# Story key → list of tasks
# Keys from the completed story creation run (KAN-19 through KAN-70)
STORIES = [
    ...
    },
    {
        ...
        "tasks": [
            ...
            {"summary": "[Backend] Anonymize billing records: nullify identifying fields, retain amounts for legal compliance", "labels": ["backend"]},
    ...
    },
    {
        ...
        "tasks": [
            ...
            {"summary": "[Backend] POST /intake/{id}/match — intake_id to AI matching service, returns top 3 therapists with scores", "labels": ["backend"]},
    ...
    # ---- KAN-16: AI Services & Abstraction Layer ----
    {
        ...
        "tasks": [
            ...
            {"summary": "[Backend] PHI safety: user IDs anonymized in all prompts (UUID only, no identifying info sent to AI)", "labels": ["backend"]},
...
def main():
    ...
    print(f"Creating subtasks for {len(STORIES)} stories...\n")

    for story in STORIES:
        story_key = story["key"]
        print(f"\n{'='*70}")
        print(f"Story {story_key}: {story['summary']}")
        print(f"{'='*70}")

        for task_data in story["tasks"]:
            key = create_subtask(
    ...
    print(f"\n{'='*70}")
    print(f"DONE")
    print(f"  Subtasks created : {total_tasks}")
